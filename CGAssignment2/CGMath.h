...
float implicitLineEq(VEC_2D a, VEC_2D b, VEC_2D p) {
...
VEC_3D barycentric(VEC_2D a, VEC_2D b, VEC_2D c, VEC_2D p) {
...
VEC_4D ndcConvert(VEC_4D points, RASTER_WINDOW win) {
	return { (points.x + 1) * win.windowWidth / 2 , (1 + points.y) * win.windowHeight / 2, points.z };
}
